...
class GuruFactorBuilder {
    ...
    public function run_upgrade_routine() {
        ...
        if (get_option('gfb_company_name') === false) {
            ...
            update_option('gfb_company_description', '');
        }
        ...
        <div class="wrap">
            ...
            <form method="post">
                ...
                <table class="form-table">
                    ...
                    <tr><th><label for="gfb_company_description">توضیحات شرکت</label></th><td><textarea id="gfb_company_description" name="gfb_company_description" rows="4" class="large-text"><?php echo esc_textarea(get_option('gfb_company_description')); ?></textarea></td></tr>
                </table>
    ...
    public function send_invoice_email_ajax() {
        ...
        $body = $this->generate_invoice_html($invoice, false);
    ...
    private function generate_invoice_html($invoice_data, $with_print_script = true) {
        ...
        $company = ['name' => get_option('gfb_company_name'),'address' => nl2br(get_option('gfb_company_address')),'phone' => get_option('gfb_company_phone'),'logo' => get_option('gfb_logo_url'),'description' => nl2br(get_option('gfb_company_description'))];
        ...
        ob_start();
        ?>
        <!DOCTYPE html><html dir="rtl"><head><meta charset="UTF-8"><title>فاکتور <?php echo esc_html($invoice_data->invoice_number); ?></title><style>@import url('https://fonts.googleapis.com/css2?family=Vazirmatn:wght@400;700&display=swap');body{font-family:'Vazirmatn',Tahoma,sans-serif;direction:rtl;text-align:right;background:white;color:#161616;padding:15px}.invoice-container{max-width:800px;margin:auto}.inv-header{display:flex;justify-content:space-between;align-items:flex-start;border-bottom:2px solid #BB000E;padding-bottom:15px;margin-bottom:20px}.inv-header .logo img{max-height:70px}.inv-header .company-info{text-align:left;font-size:.9em}.inv-title{background:#BB000E;color:white;padding:10px;text-align:center;font-size:1.2em;font-weight:bold;margin-bottom:20px}.inv-details,.inv-customer{display:flex;justify-content:space-between;margin-bottom:15px}.inv-table{width:100%;border-collapse:collapse;margin:20px 0;font-size:.95em}.inv-table th,.inv-table td{border:1px solid #EEE;padding:10px;text-align:center}.inv-table th{background:#54595F;color:white}.inv-totals-section{width:60%;margin-right:auto;text-align:left}.inv-totals-section div{display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid #EEE}.inv-footer{display:flex;justify-content:space-between;margin-top:30px;padding-top:20px;border-top:1px solid #BB000E;font-size:.85em}.inv-description{margin-top:20px;padding-top:20px;border-top:1px solid #EEE;font-size:.85em}.signature-img{max-height:80px;margin-top:10px}@media print{body{padding:0;margin:0;}}</style></head><body><div class="invoice-container"><div class="inv-header"><div class="logo"><?php if($company['logo']) printf('<img src="%s" alt="Logo">', esc_url($company['logo'])); ?></div><div class="company-info"><strong><?php echo esc_html($company['name']); ?></strong><br><?php echo wp_kses_post($company['address']); ?><br>تلفن: <?php echo esc_html($company['phone']); ?></div></div><div class="inv-title">پیش فاکتور</div><div class="inv-details"><div><strong>شماره فاکتور:</strong> <?php echo esc_html($invoice_data->invoice_number); ?></div><div><strong>تاریخ:</strong> <?php echo esc_html($invoice_data->invoice_date); ?></div></div><div class="inv-customer"><div><strong>طرف حساب:</strong> <?php echo esc_html($invoice_data->customer_name); ?></div><div><strong>شماره تماس:</strong> <?php echo esc_html($invoice_data->customer_phone); ?></div></div><div><strong>آدرس:</strong> <?php echo nl2br(esc_html($invoice_data->customer_address)); ?></div><table class="inv-table"><thead><tr><th>ردیف</th><th>عنوان</th><th>مقدار</th><th>واحد</th><th>مبلغ واحد</th><th>تخفیف</th><th>جمع</th></tr></thead><tbody><?php echo $itemsHTML; ?></tbody></table><div class="inv-totals-section"><div><strong>مبلغ قابل پرداخت:</strong><strong><?php echo number_format_i18n($invoice_data->total_amount); ?> <?php echo esc_html($invoice_data->currency); ?></strong></div></div><div class="inv-footer"><div class="bank-info"><strong>اطلاعات پرداخت:</strong><br>بانک <?php echo esc_html($bank['name']); ?> بنام <?php echo esc_html($bank['holder']); ?><br>کارت: <?php echo esc_html($bank['card']); ?><br>شبا: <?php echo esc_html($bank['sheba']); ?></div><div class="signatures" style="text-align:center"><strong>امضای فروشنده</strong><br><?php if($signature) printf('<img src="%s" alt="امضا" class="signature-img">', esc_url($signature)); else echo '<br><br>...........................'; ?></div></div><?php if (!empty($company['description'])): ?><div class="inv-description"><strong>توضیحات:</strong><br><?php echo wp_kses_post($company['description']); ?></div><?php endif; ?></div><?php if ($with_print_script): ?><script type="text/javascript">window.onload=function(){window.print();}</script><?php endif; ?></body></html>
        <?php
        return ob_get_clean();
    }
    ...
    public function save_invoice_ajax() {
        ...
        $invoice_html = $this->generate_invoice_html($invoice_data_obj, true);
    ...
    private function save_settings() {
        $settings_map = [
            'gfb_company_name' => 'sanitize_text_field',
            'gfb_company_address' => 'sanitize_textarea_field',
            'gfb_company_phone' => 'sanitize_text_field',
            'gfb_logo_url' => 'esc_url_raw',
            'gfb_signature_url' => 'esc_url_raw',
            'gfb_bank_card' => 'sanitize_text_field',
            'gfb_bank_account' => 'sanitize_text_field',
            'gfb_bank_name' => 'sanitize_text_field',
            'gfb_account_holder' => 'sanitize_text_field',
            'gfb_sheba_number' => 'sanitize_text_field',
            'gfb_default_currency' => 'sanitize_text_field',
            'gfb_company_description' => 'sanitize_textarea_field'
        ];
    
        foreach ($settings_map as $option_name => $sanitization_function) {
            if (isset($_POST[$option_name])) {
                $value = call_user_func($sanitization_function, wp_unslash($_POST[$option_name]));
